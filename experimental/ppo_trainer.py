...
def make_step_rewards(logits, token_masks, step_lengths, response_length):
...
def get_stepwise_reward(
    ...
    for i in range(query.shape[0]):
        ...
        conversation_str = rm_processing_class.apply_chat_template(
            messages, 
            tokenize=False, 
        ...
        reduced_score, full_score = make_step_rewards(outputs[0], token_masks, step_lengths, response_length)
        scores.extend(full_score)
        reduced_scores.extend(reduced_score)
    scores = torch.stack(scores)
    reduced_scores = torch.stack(reduced_scores)
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                    ...
                    if args.prm:
                        reduced_score, score = get_stepwise_reward(
                ...
                reduced_scores = torch.cat(reduced_scores, 0)
            ...
            with torch.no_grad():
                ...
                if args.prm:
                    metrics["objective/scores"] = self.accelerator.gather_for_metrics(reduced_scores.mean()).mean().item()
                else:
        ...
        with unwrap_model_for_generation(self.model, self.accelerator) as unwrapped_model:
            for batch in self.eval_dataloader:
                ...
                with torch.no_grad():
                    ...
                    postprocessed_query_response = torch.cat((query, postprocessed_response), 1)
                    if args.prm:
                        score, _ = get_stepwise_reward(self.reward_model, query, postprocessed_response, processing_class.pad_token_id, context_length, args.response_length, self.processing_class, self.rm_processing_class)
                    else:
                        _, score, _ = get_reward(
